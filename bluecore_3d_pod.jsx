/*
 * BlueCore — desalination pod.
 *
 * Loaded straight from index.html and compiled in the browser by Babel
 * standalone, so there is no build step and GitHub Pages can serve this
 * file as-is. React and THREE arrive as globals from the UMD bundles that
 * index.html loads first — that is why this file has no import/export
 * statements.
 *
 * PARTS below is the single source of truth: it places the hover markers
 * on the 3D model AND fills the "How the pod works" cards, so the copy in
 * those two places can never drift apart.
 */

const { useRef, useEffect, useState } = React;

const PARTS = [
  { pos: [0, 1.65, 0], title: "Sensor Dome", text: "Monitors salinity, flow rate and water quality in real time." },
  { pos: [2.6, 0.1, 0], title: "Solar Collar", text: "Powers the pod and keeps it buoyant right at the waterline." },
  { pos: [2.0, -1.8, 0], title: "Membrane Core", text: "Reverse-osmosis stack that desalinates the intake water." },
  { pos: [3.6, 1.0, 0], title: "Outflow Pipe", text: "Delivers purified fresh water to shore or storage." },
  { pos: [0.6, -5.6, 0], title: "Waste Hopper", text: "Concentrates brine and collected debris for pickup." },
  { pos: [5.2, 1.0, 3.4], title: "Collection Dock", text: "BlueCore retrieves the waste here and ships it out for upcycling." },
];

function BlueCorePod() {
  ...
  useEffect(() => {
    ...
    scene.fog = new THREE.Fog(0x0b2e33, 14, 36);
    ...
    // ---- Ocean plane ----
    const waterGeo = new THREE.PlaneGeometry(70, 70, 70, 70);
    ...
    // ---- Hover markers, positioned from PARTS ----
    const markers = PARTS.map((part) => {
      ...
      marker.position.set(part.pos[0], part.pos[1], part.pos[2]);
      marker.userData = part;
      pod.add(marker);
      return marker;
    });
    ...
    let prevX = 0, prevY = 0;
    let theta = 0.9, phi = 1.0, radius = 15;
    ...
    function onWheel(e) {
      ...
      radius = Math.max(7, Math.min(26, radius));
      updateCamera();
      e.preventDefault();
    }
  ...
  return (
    <>
      <div ref={mountRef} className="pod-canvas" />
      <div className="part-label" style={{ opacity: label ? 1 : 0 }}>
        {label}
      </div>
    </>
  );
...
/* The six info cards under "How the pod works", built from the same PARTS. */
function PartCards() {
  return (
    <>
      {PARTS.map((part, i) => (
        <div className="card" key={part.title}>
